...
/*   Updated: 2025/03/26 01:11:32 by jynra            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_a_to_b(t_stack_node **a, t_stack_node **b) //Define a function that prepares the cheapest nodes on top of the stacks for pushing `a` nodes to stack `b`, until there are three nodes left in `a`
{
	...
	if (cheapest_node->above_medium 
		&& cheapest_node->target_node->above_medium) //If both the cheapest `a` node and its target `b` node are above the median
		rotate_both(a, b, cheapest_node);
	else if (!(cheapest_node->above_medium) 
		&& !(cheapest_node->target_node->above_medium)) //If both the cheapest `a` node and its target `b` node are below the median
...
void	min_on_top(t_stack_node **a) //Define a function that moves the smallest number to the top
{
	...
	{
		if (find_min(*a)->above_medium) //Rotate or reverse rotate according to the position of the node on the median
			ra(a, false);
		else
			rra(a, false);
	}
